/**
 * grunt-combopage
 * https://github.com/doublehelix/grunt-manifest-generator
 * https://github.com/ginano/grunt-combopage
 *
 * Copyright (c) 2013 ginano, 2015 James Price
...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('manifestGenerator', 'get all files form setting.', function() {
        ...
        // Reserved task completion handler
        //保留任务完成句柄
         ...
         */
        /**
         * Determines if manifest file generation is complete.
        ...
        /**
         * Generate a manifest file based on the file list
         * 根据文件列表生成manifest文件
         * @param  {[type]} path
         * @param  {[type]} files
         * @return {[type]}
        ...
        this.files.forEach(function(f) {
            ...
            //parse html content and create a file list
            //获取所有的html文件列表及其内容列表
            ...
            //If the file itself contains html, traverse a list of all html files. The path is relative to the path of the manifest file.
            //如果设置了包含html文件本身，就需要遍历所有的html文件列表，但是路径是相对于manifest文件的路径的
            if (OPT.includeHTML) {
                Util.log('Locate the following html files:');
            ...
            if (OPT.includeJS) {
                ...
                Util.log('Locate the following javascript files:');
                htmlSourceList.forEach(function(fileObj) {
                    var _list = fileObj.content.match(/<script\s+(?:[^>]+\s+)?src=["']?\s*([^>]+)\s*["']?[\s>\/]/ig) || [];
                    _list.forEach(function(js) {
                        var str = js.match(/src=["']?\s*([^>"']+)\s*["']?/i),
                            _path;
                        if (!str) {
                            Util.log('Could not determine source for javascript tag: "'+ js +'"', 'warn');
                            return;
                        }
                        str = str[1].trim();
                        if (/^['"]?data:/i.test(str)) {
            ...
            if (OPT.includeHtmlImage) {
                Util.log('Locate the following images from html content:');
                htmlSourceList.forEach(function(fileObj) {
                    ...
                    _list.forEach(function(img) {
                        var str = img.match(/src=["']?\s*([^>"']+)\s*["']?/i),
                            _path;
                        if (!str) {
                            Util.log('Could not determine the source for image tag: "'+ img +'"', 'warn');
                            return;
                        }
                        str = str[1].trim();
                        if (/^['"]?data:/i.test(str)) {
            ...
            if (OPT.includeCSS || OPT.includeCssImage) {
                ...
                htmlSourceList.forEach(function(fileObj) {
                    ...
                    _list.forEach(function(css) {
                        var str = css.match(/href=(?:"\s*|'\s*)?([^>"']+)\s*(?:'|"|\s)?/i),
                            _path;
                        if (!str) {
                            Util.log('Could not determine href for css tag: "'+ css +'"', 'warn');
                            return;
                        }
                        str = str[1].trim();
                        if (/^['"]?data:/i.test(str)) {
                ...
                if (OPT.includeCssImage) {
                    ...
                    htmlSourceList.forEach(function(fileObj) {
                        ...
                        _list.forEach(function(css) {
                            var str = css.match(/^url\(([^\)]+)\)$/i),
                                _path;
                            if (!str) {
                                Util.log('Could not determine url for inline css image: "'+ css +'"', 'warn');
                                return;
                            }
                            str = str[1].trim();
                            if (/^['"]?data:/i.test(str)) {
                    ...
                    cssList.forEach(function(css) {
                        ...
                        Util.getContent(_path, function(data) {
                            ...
                            _list.forEach(function(img) {
                                var str = img.match(/^url\(([^\)]+)\)$/i),
                                    __path;
                                if (!str) {
                                    Util.log('Could not determine url for css image: "'+ img +'"', 'warn');
                                    return;
                                }
                                str = str[1].trim();
                                if (/^['"]?data:/i.test(str)) {
                ...
                if (!OPT.includeCSS) {
                    ...
                    Util.log('Locate the following css files:');
            ...
            function checkDone() {
                Util.checkAllDone(delayList, function() {
                    ...
                    Util.log('Locate the following images from style content:');
